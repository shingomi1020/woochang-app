...
const BANDS_ASC_WATER = Object.keys(UNIT_PRICES_WATER)
  .map(Number)
  .sort((a, b) => a - b);
...
const HELP_DELIVERY =
  "카드를 다시 클릭하면 선택이 해제됩니다. (시내 10,000원 / 시외 20,000원 / 기타 직접 입력)";
const HELP_INSTALL =
  "장당 금액이 적용되며 총 결제금액에 포함됩니다. (기타 선택 시 직접 입력)";
const GUIDE_POST =
  "게시대현수막은 가로 7000 × 세로 900mm 고정, 장당 50,000원(설치비 포함)입니다.";
const GUIDE_NORMAL =
  "900 초과~1800 이하는 한 폭 위 단가가 적용되며, 과금 기준 세로는 매칭된 폭으로 간주됩니다. 예: 5000×901mm → 1000폭 단가 적용. 납품/설치 비용은 장당 부과되며 총 결제금액에 포함됩니다.";
const GUIDE_STICKER =
  "솔벤시트/차량용시트/PVC캘: 면적단가 40,000원/㎡ 적용. 납품/설치 비용은 장당 부과되며 총 결제금액에 포함됩니다.";
...
const pickBand = (h, asc = BANDS_ASC_WATER, desc = BANDS_DESC_WATER) => {
  if (h <= 900) {
    for (const b of desc) if (b <= h) return b;
    return asc[0];
  }
  for (const b of asc) if (b >= h) return b;
  return asc[asc.length - 1];
};

function calcPricing(material, w, h, q, rollBy = "AUTO") {
  // PET배너: 600×1800=40,000/장, 비규격: 세로 1m당 20,000원(올림)
  if (material === "PET배너") {
    if (!(q > 0)) return null;
    if (w === 600 && h === 1800) {
      const per = 40000;
    ...
    return {
      ...
      matchedHeight: null,
      unitPriceUsed: 20000,
      supplyPerItem: perItem,
      supplyTotal: perItem * q,
    };
  }
  // 솔벤현수막: 고정 900폭
  if (material === "솔벤현수막") {
  ...
  if (isSticker(material)) {
    ...
    return {
      mode: "sticker-area",
      matchedHeight: null,
      unitPriceUsed: STICKER_UNIT_PRICE,
      supplyPerItem: perItem,
      supplyTotal: perItem * q,
    };
  }
  ...
  if (waterExactPreferred) {
    ...
    if (cands.length) {
      cands.sort((a, b) => a.matchedHeight - b.matchedHeight);
      return cands[0];
    }
  }
  if (h > 1800) {
    ...
    return {
      mode: "area",
      matchedHeight: null,
      unitPriceUsed: AREA_UNIT_PRICE,
      supplyPerItem: perItem,
      supplyTotal: perItem * q,
    };
  }
  if (rollBy === "W") {
    const matchedW = pickBand(w),
      unitW = UNIT_PRICES_WATER[matchedW] || 0;
    ...
    return {
      mode: "roll",
      orientation: "W",
      matchedHeight: matchedW,
      unitPriceUsed: unitW,
      supplyPerItem: perItemW,
      supplyTotal: perItemW * q,
    };
  }
  if (rollBy === "H") {
    const matchedH = pickBand(h),
      unitH = UNIT_PRICES_WATER[matchedH] || 0;
    ...
    return {
      mode: "roll",
      orientation: "H",
      matchedHeight: matchedH,
      unitPriceUsed: unitH,
      supplyPerItem: perItemH,
      supplyTotal: perItemH * q,
    };
  }
  const matchedW = pickBand(w),
    unitW = UNIT_PRICES_WATER[matchedW] || 0;
  ...
  const perW = unitW
    ? Math.round((h * (w <= 900 ? w : matchedW) * unitW) / (matchedW * 1000))
    : Infinity;
  const perH = unitH
    ? Math.round((w * (h <= 900 ? h : matchedH) * unitH) / (matchedH * 1000))
    : Infinity;
  return perW <= perH
    ? {
        mode: "roll",
...
const woodRopeUnitByHeight = (h) =>
  !(h > 0)
    ...
    : h <= 1800
    ? 2000 +
      (([900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1800].indexOf(pickBand(h)) || 0) *
        1000)
    : 10000 + Math.ceil((h - 1800) / 100) * 1000;
const sewOrTapeUnitByArea = (w, h) =>
  !(w > 0 && h > 0) ? 0 : Math.max(2000, Math.ceil(w / 1000) * Math.ceil(h / 1000) * 1000);
...
const PostCard = memo(function ({ opt, active, onSelect }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`text-[11px] ${hasImg ? "mt-2" : "mt-0"} whitespace-nowrap tracking-tight leading-5`}
      >
        {opt.label}
      </div>
    </div>
...
const NumInput = memo(({ value, onChange, className = "", placeholder = "원/장", disabled = false }) => (
  <input
    inputMode="numeric"
    className={`w-32 border rounded-xl p-2 text-right ${className}`}
    placeholder={placeholder}
    value={value}
    onChange={(e) => onChange(e.target.value.replace(RE_NON_DIGIT, ""))}
    disabled={disabled}
  />
));
      ...
      <div className="grid grid-cols-3 gap-2 mt-2">
        {options.map((opt) => (
          <PostCard
            key={opt.id}
            opt={opt}
            active={value.id === opt.id}
            onSelect={(c) => setValue((p) => togglePick(p, c))}
          />
        ))}
      </div>
      {value.id === "기타" && (
        <div className="mt-2 flex items-center gap-2 text-sm">
          <span className="text-xs md:text-[11px] text-gray-500">{otherLabel}</span>
          <NumInput value={otherValue} onChange={(v) => onOtherChange(v)} />
          <span className="text-xs md:text-[11px] text-gray-500">원/장</span>
        </div>
      )}
      <div className="text-xs md:text-[11px] text-gray-500 mt-1">{helper}</div>
    </div>
  );
});
...
const QtyInput = memo(function ({ value, setValue }) {
  return (
    <div className="flex items-center gap-2">
      <Button
        type="button"
        onClick={() => setValue(String(clampQty((Number(value || 0) || 1) - 1)))}
      >
        −
      </Button>
    ...
    {items.map((it) => (
      <button
        ...
        className={`px-3 py-1.5 text-sm rounded-xl border ${
          value === it ? "bg-black text-white border-black" : "bg-white"
        }`}
        onClick={() => onChange(it)}
      >
        {it}
    ...
    return (
      <div className="mt-3 bg-slate-50 border rounded-xl p-3 text-sm space-y-2">
        <div className="flex items-center justify-between">
          <span>{label}</span>
          <span className="font-medium">무료 / 장</span>
        </div>
        <div className="flex items-center justify-between gap-2">
          <label className="whitespace-nowrap">
            {postId === "끈고리" ? "추가 끈고리 수량" : "추가 타공 수량"}
          </label>
          <div className="flex items-center gap-2">
            <Button
              type="button"
              onClick={() =>
                setEyeletExtraStr(String(Math.max(0, (Number(eyeletExtraStr || 0) || 0) - 1)))
              }
            >
              −
            </Button>
            <input
              inputMode="numeric"
              className={CLS.inp}
              value={eyeletExtraStr}
              onChange={(e) => setEyeletExtraStr(e.target.value.replace(RE_NON_DIGIT, ""))}
            />
            <Button
              type="button"
              onClick={() => setEyeletExtraStr(String((Number(eyeletExtraStr || 0) || 0) + 1))}
            >
              ＋
            </Button>
          </div>
        ...
        <div className="grid grid-cols-2 gap-1 text-xs text-gray-700">
          ...
          <div>
            추가: <b>{eyeletExtra}</b>개/장
          </div>
          <div>
            합계: <b>{4 + eyeletExtra}</b>개/장
          </div>
          <div>
            {postId === "끈고리" ? "총 끈고리 수" : "총 타공수"}: <b>{(4 + eyeletExtra) * qty}</b>개
          </div>
        </div>
      </div>
    );
  ...
  if (postId === "로프미싱" || postId === "양면테이프")
    return (
      <div className="mt-3 bg-slate-50 border rounded-xl p-3 text-sm space-y-2">
        <div className="flex items-center justify-between">
          <span>{label}</span>
          <span className="font-medium">{format(sewOrTapeUnitByArea(width || 0, height || 0))}원</span>
        </div>
        <div className="text-xs text-gray-600">
          계산식: ⌈가로/1000⌉ × ⌈세로/1000⌉ × 1000원 (최소 2,000원)
        </div>
      </div>
    );
  return null;
    ...
    <div className="mt-3 bg-slate-50 border rounded-xl p-3 text-sm space-y-2">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <Button
            type="button"
            onClick={() => setExtraStr(String(Math.max(0, (Number(extraStr || 0) || 0) - 1)))}
          >
            −
          </Button>
          <input
          ...
          />
          <Button type="button" onClick={() => setExtraStr(String((Number(extraStr || 0) || 0) + 1))}>
            ＋
          </Button>
        </div>
      </div>
...
function PricingForm() {
  ...
  const [wWarn, setWWarn] = useState(false),
    [hWarn, setHWarn] = useState(false);
  const [postProcess, setPostProcess] = useState({ id: "", label: "", img: "" }),
    [eyeletExtraStr, setEyeletExtraStr] = useState("0");
  const [delivery, setDelivery] = useState({ id: "", label: "", img: "" }),
    [deliveryOtherStr, setDeliveryOtherStr] = useState("");
  const [install, setInstall] = useState({ id: "", label: "", img: "" }),
    [installOtherStr, setInstallOtherStr] = useState("");
  const [accessory, setAccessory] = useState({ id: "", label: "", img: "" }),
    [suctionExtraStr, setSuctionExtraStr] = useState("0"),
    [dombo, setDombo] = useState(false),
    [stickerRoll, setStickerRoll] = useState(null),
    [domboExceeded, setDomboExceeded] = useState(false);
  ...
  const resetAll = useCallback(() => {
    ...
    setInstallOtherStr("");
    setEyeletExtraStr("0");
    setSuctionExtraStr("0");
    setDombo(false);
    setStickerRoll(null);
    setDomboExceeded(false);
  }, []);

  const prevMat = useRef(material);
  useEffect(() => {
    const { lockW, lockH, wVal, hVal } = locksForMaterial(material, bannerCustom);
    if (material !== prevMat.current) {
      resetAll();
    }
    setWStr(lockW ? wVal : "");
    setHStr(lockH ? hVal : "");
    prevMat.current = material;
  }, [material, bannerCustom, resetAll]);

  useEffect(() => {
    if (!GROUPS[group].includes(material)) setMaterial(GROUPS[group][0]);
    resetAll();
    setBannerCustom(false);
  }, [group, material, resetAll]);

  useEffect(() => {
    if (material !== "PET배너" || !bannerCustom) {
      setWWarn(false);
      setHWarn(false);
      return;
    }
    const w = Number(wStr) || 0,
      h = Number(hStr) || 0;
    if (w > PET_WIDTH_MAX) {
      setWStr(String(PET_WIDTH_MAX));
      setWWarn(true);
    } else if (wWarn) setWWarn(false);
    if (h > PET_HEIGHT_MAX) {
      setHStr(String(PET_HEIGHT_MAX));
      setHWarn(true);
    } else if (hWarn) setHWarn(false);
  ...
  useEffect(() => {
    if (isSticker(material) && dombo && stickerRoll) {
      ...
      if (w > max) {
        setWStr(String(max));
        setDomboExceeded(true);
      }
    } else if (domboExceeded) {
      setDomboExceeded(false);
    }
  ...
  const width = Number(wStr) || 0,
    ...
    qty = clampQty(qStr);
  const eyeletExtra = useMemo(
    () => Math.max(0, Math.min(Number(eyeletExtraStr) || 0, 1000)),
    [eyeletExtraStr]
  );
  const suctionExtra = useMemo(
    () => Math.max(0, Math.min(Number(suctionExtraStr) || 0, 1000)),
    [suctionExtraStr]
  );
  const pricing = useMemo(
    () =>
      isPostBanner
        ...
        : calcPricing(material, width, height, qty),
    [isPostBanner, material, width, height, qty]
  );
  ...
  const postCostPerItem = useMemo(() => {
    switch (postProcess.id) {
      case "아일렛":
      case "끈고리":
        return eyeletLikeUnitCost(eyeletExtra);
      case "각목+로프":
  return woodRopeUnitByHeight((pricing?.matchedHeight ?? height) || 0);
      case "로프미싱":
      case "양면테이프":
        return sewOrTapeUnitByArea(width || 0, height || 0);
      default:
        return 0;
    }
  }, [postProcess.id, width, height, eyeletExtra, pricing]);
  
  const accessoryPerItem = useMemo(
    () => (group === "배너" && accessory.id === "큐방" ? eyeletLikeUnitCost(suctionExtra) : 0),
    [group, accessory.id, suctionExtra]
  );
  const deliveryPerItem = useMemo(
    () =>
      isPostBanner
        ? 0
        : delivery.id === "시내"
        ? DELIVERY_RATE_IN_CITY
        : delivery.id === "시외"
        ? DELIVERY_RATE_OUT_CITY
        : delivery.id === "기타"
        ? parseWon(deliveryOtherStr)
        : 0,
    [delivery.id, deliveryOtherStr, isPostBanner]
  );
  const installPerItem = useMemo(
    () => (isPostBanner ? 0 : install.id === "기타" ? parseWon(installOtherStr) : 0),
    [install.id, installOtherStr, isPostBanner]
  );
  ...
  const onResetSizes = useCallback(() => {
    if (!isPostBanner) setWStr("");
    if (!(isPostBanner || material === "솔벤현수막")) setHStr("");
  }, [isPostBanner, material]);
  ...
  const basisRowText = pricing
    ? pricing.mode === "pet-std"
      ? `PET 규격 600×1800 · ${format(40000)}원/장`
      : pricing.mode === "pet-custom"
      ? `PET 비규격 세로 ${Math.max(1, Math.ceil((Number(hStr) || 0) / 1000))}m × ${format(20000)}원`
      : pricing.mode === "roll"
      ? `적용 폭 ${pricing.matchedHeight}폭 · ${format(pricing.unitPriceUsed)}원/ｍ`
      : pricing.mode === "sticker-area"
      ? `${material} 면적단가 ${format(pricing.unitPriceUsed)}원/㎡`
      : pricing.mode === "area"
      ? `면적단가 ${format(pricing.unitPriceUsed)}원/㎡`
      : pricing.mode === "solvent"
      ? `솔벤 900폭 · ${format(pricing.unitPriceUsed)}원/ｍ`
      : pricing.mode === "fixed"
      ? `게시대현수막 장당 고정가 ${format(pricing.unitPriceUsed)}원`
      : null
    : null;
  ...
  return (
    <div className="p-4 md:p-6 pb-28 md:pb-6 max-w-4xl mx-auto">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          <div className="space-y-3 md:space-y-4 min-w-0">
            ...
            <div>
              {/* 돔보커팅: 사이즈 위에 배치 */}
              {isSticker(material) && (
                <div className="text-xs md:text-[11px] text-gray-500 mt-1">
                  스티커/시트지 원단폭 <b>635/1050/1270/1500mm</b>
                </div>
              )}
              {isSticker(material) && (
                <div className="mt-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Button
                      type="button"
                      variant={dombo ? "solid" : "ghost"}
                      className={`${dombo ? "bg-black text-white border-black hover:bg-black" : ""}`}
                      onClick={() => setDombo((v) => !v)}
                    >
                      돔보커팅
                    </Button>
                    {dombo && (
                      <>
                        <span className="text-xs md:text-[11px] text-gray-500">원단폭</span>
                        {STICKER_BANDS_ASC.map((b) => (
                          <Button
                            key={b}
                            type="button"
                            variant={stickerRoll === b ? "solid" : "ghost"}
                            className={`${stickerRoll === b ? "bg-black text-white border-black hover:bg-black" : ""}`}
                            onClick={() => setStickerRoll(b)}
                          >
                            {b}mm
                          </Button>
                        ))}
                  ...
                  {dombo && stickerRoll && (
                    <div className="text-xs md:text-[11px] text-gray-600 mt-1">
                      선택 폭 {stickerRoll}mm 기준 좌우 50mm 여유 적용 → 최대 가로{" "}
                      <b>{stickerRoll - 100}mm</b>
                    </div>
                  )}
                  {dombo && stickerRoll && domboExceeded && (
                    <div className="text-xs text-red-600 mt-1">
                      가로는 선택한 원단폭 {stickerRoll}mm 기준 좌우 50mm 여유를 두어 최대{" "}
                      {stickerRoll - 100}mm까지만 가능합니다.
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs md:text-[11px] text-gray-500">가로</span>
                <NumInput
                  value={wStr}
                  onChange={(v) => {
                    if (isSticker(material) && dombo && stickerRoll) {
                  ...
                  className={`${CLS.inp} ${
                    currentLocks.lockW ? "bg-gray-50 text-blue-700 font-semibold opacity-100" : ""
                  }`}
                  placeholder="mm"
                  disabled={currentLocks.lockW}
                />
                <span className="text-xs md:text-[11px] text-gray-500">세로</span>
                <NumInput
                  value={hStr}
                  onChange={setHStr}
                  className={`${CLS.inp} ${
                    currentLocks.lockH ? "bg-gray-50 text-blue-700 font-semibold opacity-100" : ""
                  }`}
                  placeholder="mm"
                  disabled={currentLocks.lockH}
                />
                {(!currentLocks.lockW || !currentLocks.lockH) && (
                  <>
                    <Button type="button" className="ml-2" onClick={onResetSizes}>
                      초기화
                    </Button>
                ...
                {group === "배너" && (
                  <>
                    <Button
                      type="button"
                      variant={bannerCustom ? "solid" : "ghost"}
                      className={`ml-2 ${bannerCustom ? "bg-black text-white border-black hover:bg-black" : ""}`}
                      onClick={() => setBannerCustom((v) => !v)}
                    >
                      {bannerCustom ? "비규격 해제" : "비규격 적용"}
                    </Button>
                    <span className="text-xs md:text-[11px] text-gray-500">
                      {bannerCustom ? "직접 입력 가능" : "기본 600×1800"}
                    </span>
                  </>
                )}
              </div>
              {material === "PET배너" && bannerCustom && (wWarn || hWarn) && (
                <div className="text-xs text-red-600 mt-1">
                  {wWarn && hWarn
                    ? "가로는 최대 1800mm, 세로는 최대 10,000mm 입니다. 입력값을 상한으로 보정했습니다."
                    : wWarn
                    ? "가로는 최대 1800mm 입니다. 입력값을 상한으로 보정했습니다."
            ...
            {material === "PET배너" && (
              <div className="text-xs md:text-[11px] text-gray-500 mt-1">
                PET 원단폭 <b>600/1200/1800mm</b>, 세로 최대 <b>10,000mm</b>
              </div>
            )}
            ...
            <div>
                ...
                <>
                  <div className="mt-2 grid grid-cols-4 gap-2">
                    {[5, 10, 15, 20, 25, 30, 35, 40, 45, 50].map((n) => (
                      <Button
                        key={n}
                        variant={Number(qStr) === n ? "solid" : "ghost"}
                        className={`${
                          Number(qStr) === n ? "bg-black text-white border-black hover:bg-black" : ""
                        }`}
                        onClick={() => setQStr((p) => String(Number(p) === n ? 1 : n))}
                      >
                        {n}개
                      </Button>
                    ))}
              ...
              <p className="text-xs md:text-[11px] text-gray-500 mt-1">최대 {format(MAX_QTY)}장</p>
            </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {OPTIONS.POST.map((o) => (
                    <PostCard
                      key={o.id}
                      opt={o}
                      active={postProcess.id === o.id}
                      onSelect={(c) => setPostProcess((p) => togglePick(p, c))}
                    />
                  ))}
                </div>
                <div className="text-xs md:text-[11px] text-gray-500 mt-1">{HELP_RECLICK}</div>
                <PostDetail
                  postId={postProcess.id}
                  label={postProcess.label}
                  qty={qty}
                  width={width}
                  height={height}
                  pricing={pricing}
                  eyeletExtra={eyeletExtra}
                  eyeletExtraStr={eyeletExtraStr}
                  setEyeletExtraStr={setEyeletExtraStr}
                />
              </div>
            )}
            ...
            <AccessoryDetail
              accId={accessory.id}
              qty={qty}
              extraStr={suctionExtraStr}
              setExtraStr={setSuctionExtraStr}
            />
            <SectionCards
              title="납품"
              ...
              helper={HELP_DELIVERY}
              otherLabel="기타 금액"
              otherValue={deliveryOtherStr}
              onOtherChange={setDeliveryOtherStr}
              hidden={isPostBanner}
            />
            <SectionCards
              title="설치 및 시공"
              ...
              helper={HELP_INSTALL}
              otherLabel="기타 금액"
              otherValue={installOtherStr}
              onOtherChange={setInstallOtherStr}
              hidden={isPostBanner}
            />
          </div>
            ...
            {pricing ? (
              <>
                {rightRows(minApplied).map(([k, v]) =>
                  typeof v === "string" ? (
                    <div key={k} className="flex justify-between text-sm gap-2 break-keep">
                    ...
                    v > 0 && (
                      <div key={k} className="flex justify-between text-sm gap-2 break-keep">
                        <span className="truncate">{k}</span>
                        <span className="shrink-0">{format(v)}원</span>
                      </div>
                    )
                  )
                )}
                <div className="border-t pt-3 mt-3">
                  <div className="flex justify-between items-baseline">
                    <span className="text-lg font-semibold">결제금액</span>
                    <span className="text-2xl text-red-600 font-bold">{format(total)}원</span>
                  </div>
                  {minApplied && (
                    <div className="text-xs text-gray-600 mt-1">
                      ※ 최저 <b>공급가액</b> {format(MIN_SUPPLY)}원 적용
                    </div>
                  )}
                </div>
              </>
            ...
            <div className="overflow-hidden rounded-xl border">
              <table className="w-full text-sm border-separate border-spacing-0">
                <thead className="bg-gray-50 text-gray-600">
                  <tr>
                    <th className="px-3 py-2 text-left w-32">항목</th>
                    <th className="px-3 py-2 text-left">내역</th>
                    <th className="px-3 py-2 text-right w-40">금액</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="px-3 py-2 align-top text-gray-600">제품</td>
                    <td className="px-3 py-2 align-top font-medium">{material}</td>
                    <td className="px-3 py-2 align-top text-right text-gray-400">-</td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 align-top text-gray-600">사이즈</td>
                    <td className="px-3 py-2 align-top">
                      {currentLocks.lockW ? (
                        <span className="text-blue-700 font-semibold">{width}</span>
                      ) : (
                        width
                      )}{" "}
                      ×{" "}
                      {currentLocks.lockH ? (
                        <span className="text-blue-700 font-semibold">{height}</span>
                      ) : (
                        height
                      )}{" "}
                      mm
                      {isPostBanner && (
                        <span className="ml-2 text-xs text-green-700">(설치비 포함)</span>
                      )}
                    </td>
                    <td className="px-3 py-2 align-top text-right text-gray-400">-</td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 align-top text-gray-600">수량</td>
                    <td className="px-3 py-2 align-top">{qty}장</td>
                    <td className="px-3 py-2 align-top text-right text-gray-400">-</td>
                  </tr>
                  {basisRowText && (
                    <tr>
                      <td className="px-3 py-2 align-top text-gray-600">단가기준</td>
                      <td className="px-3 py-2 align-top">{basisRowText}</td>
                      <td className="px-3 py-2 align-top text-right text-gray-400">-</td>
                    </tr>
                  )}
                  <tr>
                    <td className="px-3 py-2 align-top text-gray-600">본판</td>
                    <td className="px-3 py-2 align-top">
                      {minApplied ? (
                        <span>
                          공급가(최저 적용)
                          <span className="ml-1 inline-flex items-center text-xs text-orange-700 border border-orange-200 bg-orange-50 rounded px-1.5 py-0.5">
                            최저가 적용
                          </span>
                        </span>
                      ) : (
                    ...
                    <td className="px-3 py-2 align-top text-right font-medium">
                      {format(
                        minApplied
                          ? supplyShown - (postSupply + accSupply + deliverySupply + installSupply)
                          : baseSupplyRaw
                      )}
                      원
                    </td>
                  ...
                  {[
                    ...
                    .map(([k, v, desc]) => (
                      <tr key={k}>
                        <td className="px-3 py-2 align-top text-gray-600">{k}</td>
                        <td className="px-3 py-2 align-top">{desc}</td>
                        <td className="px-3 py-2 align-top text-right">{format(v)}원</td>
                      </tr>
                    ))}
                  <tr>
                    <td className="px-3 py-2 align-top text-gray-600">공급가 합계</td>
                    <td className="px-3 py-2 align-top text-gray-500">
                      (본판 + 후가공 + 부속품 + 납품 + 설치)
                    </td>
                    <td className="px-3 py-2 align-top text-right font-semibold">
                      {format(supplyShown)}원
                    </td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr className="bg-gray-50">
                    <td className="px-3 py-2 align-top text-gray-600">부가세(10%)</td>
                  ...
                  <tr className="bg-gray-100">
                    <td className="px-3 py-3 align-top text-gray-900 font-semibold text-base">
                      결제금액
                    </td>
                    <td className="px-3 py-3 align-top"></td>
                    <td className="px-3 py-3 align-top text-right text-red-600 font-bold text-xl">
                      {format(total)}원
                    </td>
                  </tr>
                </tfoot>
      ...
      <div className="text-xs md:text-[11px] text-gray-500 mt-4">
        {isSticker(material) ? GUIDE_STICKER : isPostBanner ? GUIDE_POST : GUIDE_NORMAL}
      </div>
      <DevTests />
    </div>
  );
...
function shouldClearSizesOnMaterialChange(prev, next) {
  return prev === "게시대현수막" && next !== "게시대현수막";
}
...
function DevTests() {
  useEffect(() => {
    ...
    ropeCases.forEach((c) => {
      const got = woodRopeUnitByHeight(c.h);
      console.assert(got === c.want, `[DevTests] woodRopeUnitByHeight(${c.h}) expected ${c.want}, got ${got}`);
    });

    const a1 = sewOrTapeUnitByArea(50, 50),
      a2 = sewOrTapeUnitByArea(1000, 900),
      a3 = sewOrTapeUnitByArea(2000, 3000),
      a4 = sewOrTapeUnitByArea(1999, 1001);
    const _woodEff = (w, h) =>
      woodRopeUnitByHeight((calcPricing("수성현수막", w, h, 1)?.matchedHeight) ?? h);
    ...
    [0, 1, 4, 7, 10, -3].forEach((e, i) => {
      const wants = [0, 500, 2000, 3500, 5000, 0],
        got = eyeletLikeUnitCost(e);
      console.assert(got === wants[i], `[DevTests] eyeletLikeUnitCost(${e}) expected ${wants[i]}, got ${got}`);
    });
    ...
    [
      ...
      const got = pickBand(h, BANDS_ASC_WATER, BANDS_DESC_WATER);
      console.assert(got === want, `[DevTests] pickBand(${h}) expected ${want}, got ${got}`);
    });
    ...
    console.assert(
      pH && pH.mode === "roll" && pH.orientation === "H" && pH.matchedHeight === 1000 && pH.supplyTotal === 8000,
      `[DevTests] roll by H expected 8000, got ${JSON.stringify(pH)}`
    );
    console.assert(
      pW && pW.mode === "roll" && pW.orientation === "W" && pW.supplyTotal === 7208,
      `[DevTests] roll by W expected 7208, got ${JSON.stringify(pW)}`
    );
    console.assert(
      pA && pA.mode === "roll" && pA.supplyTotal === Math.min(pH.supplyTotal, pW.supplyTotal),
      `[DevTests] AUTO uses cheaper of H/W, got ${JSON.stringify(pA)}`
    );

    const p2 = calcPricing("수성현수막", 1000, 2000, 1);
    console.assert(
      p2 && p2.mode === "area" && p2.supplyTotal === 22000,
      `[DevTests] calcPricing(water,1000,2000,1) expected area/22000, got ${JSON.stringify(p2)}`
    );
    ...
    console.assert(
      ex1 && ex1.mode === "roll" && ex1.orientation === "W" && ex1.matchedHeight === 900 && ex1.supplyTotal === 35000,
      `[DevTests] exact match by W=900 should roll @900, got ${JSON.stringify(ex1)}`
    );
    const ex2 = calcPricing("수성현수막", 900, 1200, 1);
    console.assert(
      ex2 && ex2.mode === "roll" && ex2.matchedHeight === 900 && ex2.supplyTotal === 8400,
      `[DevTests] both match 900/1200 choose 900, got ${JSON.stringify(ex2)}`
    );
    const ex3 = calcPricing("현수막배너", 900, 5000, 1);
    console.assert(ex3 && ex3.mode === "roll" && ex3.matchedHeight === 900, `[DevTests] banner-water exact match should roll, got ${JSON.stringify(ex3)}`);
    const ex4 = calcPricing("수성현수막", 1300, 1800, 1);
    console.assert(
      ex4 && ex4.mode === "roll" && ex4.matchedHeight === 1300 && ex4.supplyTotal === 27000,
      `[DevTests] both match 1300/1800 choose 1300, got ${JSON.stringify(ex4)}`
    );
    const ex5 = calcPricing("수성현수막", 1550, 1800, 1);
    console.assert(
      ex5 && ex5.mode === "roll" && ex5.matchedHeight === 1600 && ex5.supplyTotal === 28800,
      `[DevTests] 1550×1800 should pick 1600 band, got ${JSON.stringify(ex5)}`
    );
    const ex6 = calcPricing("수성현수막", 901, 2000, 1);
    console.assert(
      ex6 && ex6.mode === "roll" && ex6.orientation === "W" && ex6.matchedHeight === 1000,
      `[DevTests] 901×2000 should pick 1000 band by W, got ${JSON.stringify(ex6)}`
    );
    ...
    console.assert(nb && nb.mode === "area", `[DevTests] non-target water-like should stay area, got ${JSON.stringify(nb)}`);

    const ps = calcPricing("솔벤현수막", 2000, 900, 1),
      ps2 = calcPricing("솔벤현수막", 2000, 0, 1);
    console.assert(
      ps && ps.mode === "solvent" && ps.matchedHeight === 900 && ps.supplyTotal === 30000,
      `[DevTests] calcPricing(solvent,2000,900,1) expected solvent/900/30000, got ${JSON.stringify(ps)}`
    );
    console.assert(
      ps2 && ps2.mode === "solvent" && ps2.supplyTotal === 30000,
      `[DevTests] calcPricing(solvent,2000,0,1) should also work with fixed 900 height`
    );
    ...
    const per = 50000;
    console.assert(per * 1 === 50000, "[DevTests] post-banner 1장 expected 50000, got " + per * 1);
    console.assert(per * 3 === 150000, "[DevTests] post-banner 3장 expected 150000, got " + per * 3);

    console.assert(
      shouldClearSizesOnMaterialChange("게시대현수막", "수성현수막") === true,
      "[DevTests] shouldClearSizes post→water should be true"
    );
    console.assert(
      shouldClearSizesOnMaterialChange("수성현수막", "수성현수막") === false,
      "[DevTests] shouldClearSizes water→water should be false"
    );
    console.assert(
      shouldClearSizesOnMaterialChange("수성현수막", "솔벤현수막") === false,
      "[DevTests] shouldClearSizes water→solvent should be false"
    );

    // Group membership sanity
    console.assert(
      GROUPS["배너"].includes("현수막배너") && GROUPS["현수막"].includes("수성현수막"),
      "[DevTests] GROUPS membership basic"
    );
    console.assert(
      GROUPS["배너"][0] === "PET배너" &&
        GROUPS["배너"][1] === "부직포배너" &&
        GROUPS["배너"][2] === "현수막배너",
      "[DevTests] 배너 제품 순서 PET→부직포→현수막배너"
    );
    ...
    const pb = calcPricing("현수막배너", 1000, 901, 1);
    console.assert(pb && pb.mode === "roll" && pb.matchedHeight === 1000 && pb.supplyTotal === 8000, "[DevTests] calcPricing(현수막배너,1000,901,1) behaves like water roll");

    // PET rules
    const ppet1 = calcPricing("PET배너", 600, 1800, 1),
      ppet2 = calcPricing("PET배너", 600, 2500, 2),
      ppet3 = calcPricing("PET배너", 2001, 1000, 1),
      ppet4 = calcPricing("PET배너", 600, 10001, 1);
    console.assert(
      ppet1 && ppet1.mode === "pet-std" && ppet1.supplyTotal === 40000,
      `[DevTests] PET 규격 expected 40000, got ${JSON.stringify(ppet1)}`
    );
    console.assert(
      ppet2 && ppet2.mode === "pet-custom" && ppet2.supplyTotal === 120000,
      `[DevTests] PET 비규격 2장(2.5m→3m×20000) expected 120000, got ${JSON.stringify(ppet2)}`
    );
    console.assert(ppet3 === null, "[DevTests] PET 가로>1800 제작 불가");
    console.assert(ppet4 === null, "[DevTests] PET 세로>10m 제작 불가");

    // Sticker rules (all sticker-like materials behave the same)
    ["솔벤시트", "차량용시트", "PVC캘"].forEach((M) => {
      const s1 = calcPricing(M, 1000, 1000, 1),
        s2 = calcPricing(M, 1500, 500, 2),
        s3 = calcPricing(M, 1000, 901, 1);
      console.assert(
        s1 && s1.mode === "sticker-area" && s1.supplyTotal === 40000,
        `[DevTests] ${M} 1㎡ × 40,000원 expected 40,000, got ${s1?.supplyTotal}`
      );
      console.assert(
        s2 && s2.mode === "sticker-area" && s2.supplyTotal === 60000,
        `[DevTests] ${M} 0.75㎡×2 expected 60,000, got ${s2?.supplyTotal}`
      );
