...
const Card = memo(({ className = "", children }) => (
  <div className={cn("border rounded-2xl shadow-sm bg-white", className)}>{children}</div>
));
...
const Button = memo(({ className = "", variant = "ghost", ...props }) => (
  <button
    ...
    className={cn(
      "inline-flex items-center justify-center rounded-xl border px-3 py-2 md:py-1.5 text-sm min-h-[40px] md:min-h-[36px] active:scale-95 transition",
      variant === "ghost" && "hover:bg-gray-50",
      variant === "solid" && "bg-black text-white border-black hover:bg-black",
      className
    )}
  />
));
...
const UNIT_PRICES_WATER = {
  ...
  1000: 8000, 1100: 10000, 1200: 10000, 1300: 15000, 1400: 15000, 1500: 15000, 1600: 16000, 1800: 17000,
};
...
const STICKER_MATS = GROUPS["스티커/시트지"];
...
const GUIDE_NORMAL = "900 초과~1800 이하는 한 폭 위 단가가 적용되며, 과금 기준 세로는 매칭된 폭으로 간주됩니다. 예: 5000×901mm → 1000폭 단가 적용. 납품/설치 비용은 장당 부과되며 총 결제금액에 포함됩니다.";
...
/** 밴드 매칭: 항상 상향(ceil) 매칭 */
const pickBand = (h, asc = BANDS_ASC_WATER) => {
  for (const b of asc) if (h <= b) return b;
  return asc[asc.length - 1];
};

/* ========================== Pricing ========================== */
function calcPricing(material, w, h, q, rollBy = "AUTO") {
  // PET배너: 600×1800 규격가, 비규격 세로 1m 올림×20,000
  if (material === "PET배너") {
    if (!(q > 0)) return null;
    if (w === 600 && h === 1800) {
      const per = 40000;
  ...
  if (waterExactPreferred) {
    ...
    if (bandW && w <= bandW && bandW <= 1800) {
      const unit = UNIT_PRICES_WATER[bandW] || 0;
      if (unit) {
        const eff = w <= 900 ? w : bandW;
        const per = Math.round((h * eff * unit) / (bandW * 1000));
        cands.push({ mode: "roll", orientation: "W", matchedHeight: bandW, unitPriceUsed: unit, supplyPerItem: per, supplyTotal: per * q });
      }
    }
    // H 기준 후보
    if (bandH && h <= bandH && bandH <= 1800) {
      const unit = UNIT_PRICES_WATER[bandH] || 0;
      if (unit) {
        const eff = h <= 900 ? h : bandH;
        const per = Math.round((w * eff * unit) / (bandH * 1000));
        cands.push({ mode: "roll", orientation: "H", matchedHeight: bandH, unitPriceUsed: unit, supplyPerItem: per, supplyTotal: per * q });
      }
    }
    // 정확 일치(있는 경우)
    if (BANDS_ASC_WATER.includes(w) && w <= 1800) {
      ...
      if (u) {
        const per = Math.round((h * (w <= 900 ? w : w) * u) / (w * 1000));
        cands.push({ mode: "roll", orientation: "W", matchedHeight: w, unitPriceUsed: u, supplyPerItem: per, supplyTotal: per * q });
      }
    }
    if (BANDS_ASC_WATER.includes(h) && h <= 1800) {
      const u = UNIT_PRICES_WATER[h] || 0;
      if (u) {
        const per = Math.round((w * (h <= 900 ? h : h) * u) / (h * 1000));
        cands.push({ mode: "roll", orientation: "H", matchedHeight: h, unitPriceUsed: u, supplyPerItem: per, supplyTotal: per * q });
      }
    }
    if (cands.length) {
  ...
  // 수동/자동 비교
  const by = (ori) => {
    const mh = ori === "W" ? pickBand(w) : pickBand(h);
    const unit = UNIT_PRICES_WATER[mh] || 0;
    if (!mh || !unit) return null;
    const eff = (ori === "W" ? (w <= 900 ? w : mh) : (h <= 900 ? h : mh));
    const per = Math.round(((ori === "W" ? h * eff : w * eff) * unit) / (mh * 1000));
    return { mode: "roll", orientation: ori, matchedHeight: mh, unitPriceUsed: unit, supplyPerItem: per, supplyTotal: per * q };
  };
  if (rollBy === "W") return by("W");
  if (rollBy === "H") return by("H");
  const wPick = by("W"), hPick = by("H");
  if (!wPick && !hPick) return null;
  return !hPick || (wPick && wPick.supplyPerItem <= hPick.supplyPerItem) ? wPick : hPick;
}
...
const woodRopeUnitByHeight = (h) =>
  ...
  : h <= 900 ? 2000
  : h <= 1800
  ? 2000 + ([900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1800].indexOf(pickBand(h)) || 0) * 1000
...
const PostCard = memo(function ({ opt, active, onSelect }) {
  ...
  return (
    <div
      ...
      className={cn(
        "cursor-pointer border rounded-xl p-3 text-center select-none transition w-full h-full",
    ...
    >
      {hasImg && (
        <div className="relative w-full h-24 md:h-28 rounded-lg overflow-hidden bg-gray-100">
          <img
            loading="lazy" decoding="async" src={opt.img} alt={opt.label}
            className="absolute inset-0 w-full h-full object-cover"
            onError={() => setImgOk(false)}
          />
        </div>
      )}
      <div className={cn("text-[11px] whitespace-nowrap tracking-tight leading-5", hasImg ? "mt-2" : "mt-0")}>
        {opt.label}
      </div>
    </div>
  );
...
const NumInput = memo(({ value, onChange, className = "", placeholder = "원/장", disabled = false }) => (
  <input
    inputMode="numeric"
    className={cn("w-32 border rounded-xl p-2 text-right", className)}
    ...
    <div className="mt-6">
      ...
      {value.id === "기타" && (
        <div className="mt-2 flex items-center gap-2 text-sm">
          ...
          <NumInput value={otherValue} onChange={onOtherChange} />
          <span className={CLS.small}>원/장</span>
        </div>
      )}
      <div className={cn(CLS.small, "mt-1")}>{helper}</div>
    </div>
  );
});
    ...
    <div className="flex flex-wrap gap-2 border-b mb-4 pb-2">
      {tabs.map((t) => (
        <button
          ...
          onClick={() => onChange(t)}
          className={cn(
            "px-4 py-2 text-sm font-medium transition-colors border-b-2 rounded-t-xl",
  ...
  <div className="flex gap-2">
    {items.map((it) => (
      <button
        key={it}
        type="button"
        className={cn("px-3 py-1.5 text-sm rounded-xl border", value === it ? "bg-black text-white border-black" : "bg-white")}
        onClick={() => onChange(it)}
      >
        {it}
      </button>
...
const PostDetail = memo(function ({ postId, label, qty, width, height, pricing, eyeletExtra, eyeletExtraStr, setEyeletExtraStr }) {
      ...
      <div className="mt-3 bg-slate-50 border rounded-xl p-3 text-sm space-y-2">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <input inputMode="numeric" className={CLS.inp} value={eyeletExtraStr} onChange={(e) => setEyeletExtraStr(e.target.value.replace(RE_NON_DIGIT, ""))} />
  ...
  if (postId === "로프미싱" || postId === "양면테이프")
    return (
      <div className="mt-3 bg-slate-50 border rounded-xl p-3 text-sm space-y-2">
        <div className="flex items-center justify-between"><span>{label}</span><span className="font-medium">{format(sewOrTapeUnitByArea(width || 0, height || 0))}원</span></div>
    ...
    <div className="mt-3 bg-slate-50 border rounded-xl p-3 text-sm space-y-2">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <input inputMode="numeric" className={CLS.inp} value={extraStr} onChange={(e) => setExtraStr(e.target.value.replace(RE_NON_DIGIT, ""))} />
...
function PricingForm() {
  const [activeTab, setActiveTab] = useState("실사출력");
  ...
  const [wStr, setWStr] = useState(""), [hStr, setHStr] = useState(""), [qStr, setQStr] = useState("1");
  ...
  const pricing = useMemo(
    () => isPostBanner ? { mode: "fixed", unitPriceUsed: 50000, supplyPerItem: 50000, supplyTotal: 50000 * qty }
  ...
  const deliveryPerItem = useMemo(
    () => isPostBanner ? 0
      : delivery.id === "시내" ? DELIVERY_RATE_IN_CITY
      : delivery.id === "시외" ? DELIVERY_RATE_OUT_CITY
      : delivery.id === "기타" ? parseWon(deliveryOtherStr) : 0,
  ...
  const { baseSupplyRaw, baseShown, postSupply, accSupply, deliverySupply, installSupply, supplyShown, vat, total, minApplied } = useMemo(() => {
  ...
  const note = useMemo(() => {
    ...
    switch (pricing.mode) {
      case "pet-std": return "PET배너: 규격 600×1800 40,000원/장";
      case "pet-custom": return "PET배너: 비규격 세로 1m당 20,000원";
      case "solvent": return "솔벤현수막: 900폭 단일, 단가 15,000원/ｍ";
      case "sticker-area": return `${material}: 면적단가(40,000원/㎡) 적용`;
      case "area": return "1800폭 초과: 면적단가(11,000원/㎡) 적용";
      case "roll": default: return `${pricing.matchedHeight}폭 단가(${format(pricing.unitPriceUsed)}원/ｍ) 적용`;
    }
  ...
  const basisRowText = pricing
    ...
    : pricing.mode === "fixed" ? `게시대현수막 장당 고정가 ${format(pricing.unitPriceUsed)}원` : null
    : null;
  ...
  return (
    <div className="p-4 md:p-6 pb-28 md:pb-6 max-w-4xl mx-auto">
      ...
      <Card className="shadow-md">
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          <div className="space-y-3 md:space-y-4 min-w-0">
            ...
            <div>
              {/* 돔보커팅: 사이즈 위 */}
              {isSticker(material) && <div className={cn(CLS.small, "mt-1")}>스티커/시트지 원단폭 <b>635/1050/1270/1500mm</b></div>}
              {isSticker(material) && (
                <div className="mt-3">
                  <label className="block text-sm font-medium">돔보 커팅</label>
                  <div className="flex flex-wrap items-center gap-2">
                    <Button type="button" variant={dombo ? "solid" : "ghost"} onClick={() => setDombo((v) => !v)}>돔보커팅</Button>
                    {dombo && (
                      <>
                        <span className={CLS.small}>원단폭</span>
                        {STICKER_BANDS_ASC.map((b) => (
                          <Button key={b} type="button" variant={stickerRoll === b ? "solid" : "ghost"} onClick={() => setStickerRoll(b)}>{b}mm</Button>
                        ))}
                      </>
                    )}
                  </div>
                  {dombo && stickerRoll && (
                    <div className={cn(CLS.small, "mt-1")}>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <NumInput
                  ...
                  className={cn(CLS.inp, currentLocks.lockW && "bg-gray-50 text-blue-700 font-semibold opacity-100")}
                  placeholder="mm"
                ...
                <NumInput
                  value={hStr}
                  onChange={setHStr}
                  className={cn(CLS.inp, currentLocks.lockH && "bg-gray-50 text-blue-700 font-semibold opacity-100")}
                  placeholder="mm"
                ...
                {(!currentLocks.lockW || !currentLocks.lockH) && (
                  <>
                    ...
                    <span className={CLS.small}>안전 한도: {format(MAX_MM)}mm</span>
                  </>
                )}
                {isPostBanner && <span className={CLS.badge}>설치비 포함</span>}
                {group === "배너" && (
                  <>
                    <Button type="button" variant={bannerCustom ? "solid" : "ghost"} className="ml-2" onClick={() => setBannerCustom((v) => !v)}>
            ...
            {material === "PET배너" && <div className={cn(CLS.small, "mt-1")}>PET 원단폭 <b>600/1200/1800mm</b>, 세로 최대 <b>10,000mm</b></div>}
                ...
                <>
                  <div className="mt-2 grid grid-cols-4 gap-2">
                    {[5,10,15,20,25,30,35,40,45,50].map((n) => (
                      <Button key={n} variant={Number(qStr) === n ? "solid" : "ghost"} onClick={() => setQStr((p) => String(Number(p) === n ? 1 : n))}>{n}개</Button>
                    ))}
                  </div>
                  <div className={cn(CLS.small, "mt-1")}>빠른 선택</div>
                </>
              )}
              <p className={cn(CLS.small, "mt-1")}>최대 {format(MAX_QTY)}장</p>
            </div>
            ...
            {showPostProcess && (
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {OPTIONS.POST.map((o) => (
                    <PostCard key={o.id} opt={o} active={postProcess.id === o.id} onSelect={(c) => setPostProcess((p) => togglePick(p, c))} />
                  ))}
                </div>
                <div className={cn(CLS.small, "mt-1")}>{HELP_RECLICK}</div>
                <PostDetail
                  postId={postProcess.id} label={postProcess.label} qty={qty}
            ...
            <SectionCards title="부속품" options={OPTIONS.ACC} value={accessory} setValue={setAccessory} helper={HELP_RECLICK}
                          otherLabel="" otherValue="" onOtherChange={() => {}} hidden={group !== "배너"} />
            <AccessoryDetail accId={accessory.id} qty={qty} extraStr={suctionExtraStr} setExtraStr={setSuctionExtraStr} />
            <SectionCards title="납품" options={OPTIONS.DELIVERY} value={delivery} setValue={setDelivery} helper={HELP_DELIVERY}
                          otherLabel="기타 금액" otherValue={deliveryOtherStr} onOtherChange={setDeliveryOtherStr} hidden={isPostBanner} />
            <SectionCards title="설치 및 시공" options={OPTIONS.INSTALL} value={install} setValue={setInstall} helper={HELP_INSTALL}
                          otherLabel="기타 금액" otherValue={installOtherStr} onOtherChange={setInstallOtherStr} hidden={isPostBanner} />
          </div>
          ...
          {pricing ? (
            <div className="overflow-hidden rounded-xl border">
              <table className="w-full text-sm border-separate border-spacing-0">
                ...
                <tbody>
                  <tr><td className="px-3 py-2 align-top text-gray-600">제품</td><td className="px-3 py-2 align-top font-medium">{material}</td><td className="px-3 py-2 align-top text-right text-gray-400">-</td></tr>
                  <tr>
                    <td className="px-3 py-2 align-top text-gray-600">사이즈</td>
                    <td className="px-3 py-2 align-top">
                      {currentLocks.lockW ? <span className="text-blue-700 font-semibold">{width}</span> : width} × {currentLocks.lockH ? <span className="text-blue-700 font-semibold">{height}</span> : height} mm
                      {isPostBanner && <span className="ml-2 text-xs text-green-700">(설치비 포함)</span>}
                    </td>
                    <td className="px-3 py-2 align-top text-right text-gray-400">-</td>
                  </tr>
                  <tr><td className="px-3 py-2 align-top text-gray-600">수량</td><td className="px-3 py-2 align-top">{qty}장</td><td className="px-3 py-2 align-top text-right text-gray-400">-</td></tr>
                  {basisRowText && <tr><td className="px-3 py-2 align-top text-gray-600">단가기준</td><td className="px-3 py-2 align-top">{basisRowText}</td><td className="px-3 py-2 align-top text-right text-gray-400">-</td></tr>}
                  <tr>
                    <td className="px-3 py-2 align-top text-gray-600">본판</td>
                    <td className="px-3 py-2 align-top">
                      {minApplied ? (<span>공급가(최저 적용)<span className="ml-1 inline-flex items-center text-xs text-orange-700 border border-orange-200 bg-orange-50 rounded px-1.5 py-0.5">최저가 적용</span></span>)
                                 : (<span>공급가</span>)}
                    </td>
                  ...
                  ].filter(([, v]) => v > 0).map(([k, v, desc]) => (
                    <tr key={k}><td className="px-3 py-2 align-top text-gray-600">{k}</td><td className="px-3 py-2 align-top">{desc}</td><td className="px-3 py-2 align-top text-right">{format(v)}원</td></tr>
                  ))}
                  <tr>
                    <td className="px-3 py-2 align-top text-gray-600">공급가 합계</td>
                    <td className="px-3 py-2 align-top text-gray-500">(본판 + 후가공 + 부속품 + 납품 + 설치)</td>
                    <td className="px-3 py-2 align-top text-right font-semibold">{format(supplyShown)}원</td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr className="bg-gray-50"><td className="px-3 py-2 align-top text-gray-600">부가세(10%)</td><td className="px-3 py-2 align-top"></td><td className="px-3 py-2 align-top text-right font-medium">{format(vat)}원</td></tr>
                  <tr className="bg-gray-100"><td className="px-3 py-3 align-top text-gray-900 font-semibold text-base">결제금액</td><td className="px-3 py-3 align-top"></td><td className="px-3 py-3 align-top text-right text-red-600 font-bold text-xl">{format(total)}원</td></tr>
                </tfoot>
              </table>
            </div>
          ) : (
      ...
      <div className={cn(CLS.small, "mt-4")}>
...
function locksForMaterial(m, bannerCustom = false) {
  if (m === "게시대현수막") return { lockW: true, lockH: true, wVal: "7000", hVal: "900" };
  if (m === "솔벤현수막") return { lockW: false, lockH: true, wVal: "", hVal: "900" };
  if (GROUPS["배너"].includes(m)) return bannerCustom ? { lockW: false, lockH: false, wVal: "", hVal: "" } : { lockW: true, lockH: true, wVal: "600", hVal: "1800" };
  return { lockW: false, lockH: false, wVal: "", hVal: "" };
}

function shouldClearSizesOnMaterialChange(prev, next) { return prev === "게시대현수막" && next !== "게시대현수막"; }

/* ========================== DevTests (unchanged + 보강) ========================== */
function DevTests() {
  useEffect(() => {
    ...
    [0, 1, 4, 7, 10, -3].forEach((e, i) => {
      const wants = [0, 500, 2000, 3500, 5000, 0], got = eyeletLikeUnitCost(e);
      console.assert(got === wants[i], `[DevTests] eyeletLikeUnitCost(${e}) expected ${wants[i]}, got ${got}`);
    });

    [[10, 300], [900, 900], [901, 1000], [1000, 1000], [1799, 1800], [1800, 1800]].forEach(([h, want]) => {
      const got = pickBand(h, BANDS_ASC_WATER);
    ...
    console.assert(ex1 && ex1.mode === "roll" && ex1.orientation === "W" && ex1.matchedHeight === 900 && ex1.supplyTotal === 35000, `[DevTests] exact match by W=900 should roll @900, got ${JSON.stringify(ex1)}`);
    const ex2 = calcPricing("수성현수막", 900, 1200, 1);
    console.assert(ex2 && ex2.mode === "roll" && ex2.matchedHeight === 900 && ex2.supplyTotal === 8400, `[DevTests] both match 900/1200 choose 900, got ${JSON.stringify(ex2)}`);
    ...
    console.assert(ex4 && ex4.mode === "roll" && ex4.matchedHeight === 1300 && ex4.supplyTotal === 27000, `[DevTests] both match 1300/1800 choose 1300, got ${JSON.stringify(ex4)}`);
    const ex5 = calcPricing("수성현수막", 1550, 1800, 1);
    console.assert(ex5 && ex5.mode === "roll" && ex5.matchedHeight === 1600 && ex5.supplyTotal === 28800, `[DevTests] 1550×1800 should pick 1600 band, got ${JSON.stringify(ex5)}`);
    const ex6 = calcPricing("수성현수막", 901, 2000, 1);
    console.assert(ex6 && ex6.mode === "roll" && ex6.orientation === "W" && ex6.matchedHeight === 1000, `[DevTests] 901×2000 should pick 1000 band by W, got ${JSON.stringify(ex6)}`);
    ...
    console.assert(ps && ps.mode === "solvent" && ps.matchedHeight === 900 && ps.supplyTotal === 30000, `[DevTests] solvent 2000×900 expected 30000, got ${JSON.stringify(ps)}`);
    console.assert(ps2 && ps2.mode === "solvent" && ps2.supplyTotal === 30000, `[DevTests] solvent fixed 900 height ok`);
    ...
    ["솔벤시트", "차량용시트", "PVC캘"].forEach((M) => {
      ...
      console.assert(s1 && s1.mode === "sticker-area" && s1.supplyTotal === 40000, `[DevTests] ${M} 1㎡×40,000 expected 40,000, got ${s1?.supplyTotal}`);
      console.assert(s2 && s2.mode === "sticker-area" && s2.supplyTotal === 60000, `[DevTests] ${M} 0.75㎡×2 expected 60,000, got ${s2?.supplyTotal}`);
