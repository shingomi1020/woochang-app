...
const pickBand=(h,asc=BANDS_ASC_WATER,desc=BANDS_DESC_WATER)=> h<=900?(desc.find(b=>b<=h)||asc[0]):(asc.find(b=>b>=h)||asc[asc.length-1]);
...
function DevTests(){useEffect(()=>{
  ...
  const ex7=calcPricing("수성현수막",2000,901,1);console.assert(ex7&&ex7.mode==="roll"&&ex7.orientation==='H'&&ex7.matchedHeight===1000,`[DevTests] 2000×901 should pick 1000 band by H, got ${JSON.stringify(ex7)}`);
